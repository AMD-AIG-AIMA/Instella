...
from hf_instella.modeling_instella import InstellaForCausalLM
from instella import ModelConfig, TrainConfig
from instella.checkpoint import build_sharded_checkpointer
from instella.util import _get_s3_client
from hf_instella.configuration_instella import InstellaConfig
...
def write_config(checkpoint_dir: str):
    ...
    for k in config_kwargs:
        if k in config:
            set_kwargs[k] = config_kwargs[k]
    
    set_kwargs["architectures"] =  ["InstellaForCausalLM"]
    set_kwargs["hidden_size"] = config_kwargs["d_model"]
    set_kwargs["intermediate_size"] = config_kwargs["mlp_hidden_size"] // 2
    set_kwargs["max_position_embeddings"] = config_kwargs["max_sequence_length"]
    set_kwargs["num_attention_heads"] = config_kwargs["n_heads"]
    set_kwargs["num_hidden_layers"] = config_kwargs["n_layers"]
    set_kwargs["num_key_value_heads"] = config_kwargs["n_heads"]
    set_kwargs["tie_word_embeddings"] = config_kwargs["weight_tying"]
    set_kwargs["pad_token_id"] = config_kwargs["pad_token_id"]
    set_kwargs["eos_token_id"] = config_kwargs["eos_token_id"]
    set_kwargs["bos_token_id"] = config_kwargs["eos_token_id"]
    set_kwargs["vocab_size"] = config_kwargs.get("embedding_size", config_kwargs["vocab_size"])

    if config_kwargs.get("layer_norm_eps"):
        set_kwargs["rms_norm_eps"] = config_kwargs["layer_norm_eps"]

    if config_kwargs.get("rope_theta"):
        set_kwargs["rope_theta"] = config_kwargs["rope_theta"]

    config = InstellaConfig(**set_kwargs)
    logger.info(f"Saving HF-compatible config to {os.path.join(checkpoint_dir, 'config.json')}")
    print(f"Saving HF-compatible config to {os.path.join(checkpoint_dir, 'config.json')}")
    config.save_pretrained(checkpoint_dir)


def write_model(checkpoint_dir: str, ignore_instella_compatibility: bool = False):
    ...
    new_state_dict = {
        key.replace(common_prefix, f"{InstellaForCausalLM.base_model_prefix}.transformer."): val
    ...
    for k in state_dict:
        if k == "transformer.wte.weight":
            new_state_dict["model.embed_tokens.weight"] = state_dict[k]
        elif "k_norm.weight" in k:
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("k_norm.weight", "self_attn.k_norm.weight")] = state_dict[k]
        elif "q_norm.weight" in k:
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("q_norm.weight", "self_attn.q_norm.weight")] = state_dict[k]
        elif "att_proj.weight" in k:
            part1, part2, part3 = torch.chunk(state_dict[k], 3)            
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("att_proj.weight", "self_attn.q_proj.weight")] = part1
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("att_proj.weight", "self_attn.k_proj.weight")] = part2
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("att_proj.weight", "self_attn.v_proj.weight")] = part3
        elif "ff_proj.weight" in k:
            part1, part2 = torch.chunk(state_dict[k], 2)            
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("ff_proj.weight", "mlp.up_proj.weight")] = part1
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("ff_proj.weight", "mlp.gate_proj.weight")] = part2
        elif "attn_out.weight" in k:
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("attn_out.weight", "self_attn.o_proj.weight")] = state_dict[k]
        elif "ff_out.weight" in k and "blocks" in k:
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("ff_out.weight", "mlp.down_proj.weight")] = state_dict[k]
        elif "attn_norm.weight" in k:
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("attn_norm.weight", "pre_attention_layernorm.weight")] = state_dict[k]
        elif "ff_norm.weight" in k:
            new_state_dict[k.replace("transformer.blocks", "model.layers").replace("ff_norm.weight", "pre_feedforward_layernorm.weight")] = state_dict[k]
        elif "ff_out.weight" in k and "blocks" not in k:
            new_state_dict[k.replace("transformer", "").replace(".ff_out.weight", "lm_head.weight")] = state_dict[k]

    new_state_dict["model.norm.weight"] = state_dict["transformer.ln_f.weight"]

    torch.save(new_state_dict, new_model_path)

    if ignore_instella_compatibility:
        os.remove(old_model_path)

def write_tokenizer(checkpoint_dir: str, input_tokenizer_path):
...
def convert_checkpoint(checkpoint_dir: str, ignore_instella_compatibility: bool = False, input_tokenizer_path = None):
    print("Converting checkpoint to HF format...")
    write_config(checkpoint_dir)

    print("Saving model to checkpoint...")
    write_model(checkpoint_dir, ignore_instella_compatibility=ignore_instella_compatibility)

    print("Saving tokenizer to checkpoint...")
    write_tokenizer(checkpoint_dir, input_tokenizer_path)
...
def validate_tokenizer(checkpoint_dir: str, tokenizer_name_or_path: Optional[str] = None, fix_tokenizer=False):
    ...
    if tokenizer_name_or_path == "allenai/gpt-neox-instella-dolma-v1_5" or tokenizer_name_or_path.endswith(
...
def main():
    ...
    parser.add_argument(
        "--checkpoint-dir",
        help="Location of Instella checkpoint.",
        required=True,
    )
    ...
    parser.add_argument(
        "--ignore-instella-compatibility",
        action="store_true",
        help="Ignore compatibility with the instella codebase. "
        "This will remove files that are needed specifically for instella codebase, eg. config.yaml, etc.",
    )
    parser.add_argument(
        "--logger-level",
        default="warning",
    ...
    parser.add_argument(
        "--fix-tokenizer",
        help="Change eos token to 50279",
    )

    parser.add_argument(
        "--keep-instella-artifacts",
        action="store_true",
        help="Keep instella-specific writ in the checkpoint.",
    )
    ...
    validate_tokenizer(checkpoint_dir=local_checkpoint_dir, tokenizer_name_or_path=args.tokenizer, fix_tokenizer=args.fix_tokenizer)
    convert_checkpoint(args.checkpoint_dir, args.ignore_instella_compatibility, args.tokenizer,)

    if not args.keep_instella_artifacts:
        ...
        os.remove(os.path.join(local_checkpoint_dir, "train.pt"))
        os.remove(os.path.join(local_checkpoint_dir, "optim.pt"))
