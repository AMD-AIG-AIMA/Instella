...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.distributed_strategy == DistributedStrategy.ddp:
        ...
        if cfg.model.init_device != "cuda":
            raise InstellaConfigurationError("DDP does not work with init_device set to anything other than `cuda`.")
        ...
        dist_model = DDP(instella_model.to(device), find_unused_parameters=cfg.ddp.find_unused_params)
        ...
        if cfg.fsdp.sharding_strategy in (ShardingStrategy.HYBRID_SHARD, ShardingStrategy._HYBRID_SHARD_ZERO2):
            ...
            if get_world_size() % num_model_replicas != 0:
                raise InstellaConfigurationError("fsdp.hybrid_sharding_num_model_replicas must divide world size")
    ...
    if cfg.seq_parallel_size > 1:
        if cfg.seq_parallel_size == dist.get_world_size():
            log.info(f"Using world as sequence parallel group")
            seq_parallel_group = dist.group.WORLD
        else:
            log.info(f"Initializing sequence parallel groups with size {cfg.seq_parallel_size}")
            seq_parallel_group, _ = dist.new_subgroups(cfg.seq_parallel_size)
        
        assert cfg.model.flash_attention, "Sequence Parallel must have flash attention enabled"
    else:
