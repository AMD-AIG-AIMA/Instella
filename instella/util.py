...
from instella_data.data import get_data_path
...
def setup_logging(log_filter_type: LogFilterType = LogFilterType.rank0_only) -> None:
    ...
    if (
        os.environ.get("Instella_NONINTERACTIVE", False)
...
def excepthook(exctype, value, traceback):
    ...
    elif issubclass(exctype, InstellaCliError):
        rich.get_console().print(f"[yellow]{value}[/]", highlight=False)
    elif issubclass(exctype, InstellaError):
...
def _get_s3_profile_name(scheme: str) -> Optional[str]:
    ...
    if scheme == "r2":
        ...
        if profile_name is None:
            raise InstellaEnvironmentError(
...
def _get_s3_endpoint_url(scheme: str) -> Optional[str]:
    if scheme == "s3":
        ...
        if s3_endpoint_url is None:
            logging.warning("s3 endpoint url is not set. Did you forget to set the 'S3_ENDPOINT_URL' env var?")
...
def _get_s3_client(scheme: str):
    ...
    return session.client(
        ...
        use_ssl=not int(os.environ.get("Instella_NO_SSL", "0")),
    )
...
def _s3_upload(
    source: Path, scheme: str, bucket_name: str, key: str, save_overwrite: bool = False, max_attempts: int = 5
):
    ...
    if not save_overwrite:
        ...
        if err is not None:
            raise InstellaNetworkError(f"Failed to check object existence during {scheme} upload") from err

    try:
        from boto3.s3.transfer import TransferConfig
        config = TransferConfig(max_concurrency=2)
        _get_s3_client(scheme).upload_file(str(source), bucket_name, key, Config=config)
    except boto_exceptions.ClientError as e:
        raise InstellaNetworkError(f"Failed to upload to {scheme}") from e


def _s3_file_size(scheme: str, bucket_name: str, key: str, max_attempts: int = 5) -> int:
    err: Optional[Exception] = None
    for attempt in range(1, max_attempts + 1):
        try:
    ...
    raise InstellaNetworkError(f"Failed to get {scheme} file size") from err


def _s3_get_bytes_range(
    scheme: str, bucket_name: str, key: str, bytes_start: int, num_bytes: int, max_attempts: int = 5
) -> bytes:
    err: Optional[Exception] = None
    for attempt in range(1, max_attempts + 1):
        try:
    ...
    raise InstellaNetworkError(f"Failed to get bytes range from {scheme}") from err
...
def save_hf_dataset_to_disk(
    ...
    to `instella_data` as follows:

    ```
    import datasets

    from instella.util import save_hf_dataset_to_disk

    path, name, split = ...

    dataset = datasets.load_dataset(path, name=name, split=split)
    save_hf_dataset_to_disk(dataset, path, name, split, "instella_data/hf_datasets")
    ```
    """
...
def load_hf_dataset(path: str, name: Optional[str], split: str):
    ...
    `save_hf_dataset_to_disk` and located in `instella_data/hf_datasets`.
    """
...
def default_thread_count() -> int:
    return int(os.environ.get("Instella_NUM_THREADS") or min(32, (os.cpu_count() or 1) + 4))
...
def threaded_generator(g, maxsize: int = 16, thread_name: Optional[str] = None):
    ...
    for x in iter(q.get, sentinel):
        if isinstance(x, Exception):
            raise InstellaThreadError(f"generator thread {thread_name} failed") from x
        else:
            yield x
