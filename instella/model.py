...
__all__ = [
    ...
    "SwiGLU",
    "InstellaBlock",
    "InstellaSequentialBlock",
    "Instella",
    "InstellaOutput",
    "InstellaGenerateOutput",
]
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, seqlen_offset: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            pos_sin, pos_cos = self.get_rotary_embedding(key_len + seqlen_offset, q_.device)
            if seqlen_offset > 0:
                assert key_len == query_len, "seqlen_offset is only used in training where key_len == query_len"
            pos_sin = pos_sin[:, :, seqlen_offset:, :]
            pos_cos = pos_cos[:, :, seqlen_offset:, :]
...
class InstellaBlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        if max_doc_len is not None and cu_doc_lens is not None:
            assert self.flash_attn_varlen_kvpacked_func is not None, "flash-attn is required for document masking"
            assert attn_mask is None, "attn-mask is currently not supported with document masking"
            B, T, D = q.size(0), q.size(2), q.size(3)
            q = q.transpose(1, 2).view(B * T, -1, D)
            k = k.transpose(1, 2).view(B * T, -1, D)
            v = v.transpose(1, 2).view(B * T, -1, D)
            ...
            if seq_parallel_group is not None and dist.is_initialized() and dist.get_world_size(seq_parallel_group) > 1:
                attention_func = self.distributed_varlen_attn_func
                kwargs = {"group": seq_parallel_group}
            else:
                attention_func = self.flash_attn_varlen_kvpacked_func
                kwargs = {}
            ...
            assert seq_parallel_group is None
    ...
    @classmethod
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> InstellaBlock:
        if config.block_type == BlockType.sequential:
            return InstellaSequentialBlock(layer_id, config, cache)
        elif config.block_type == BlockType.llama:
            return InstellaLlamaBlock(layer_id, config, cache)
        else:
            raise NotImplementedError(f"Unknown block type: '{config.block_type}'")


class InstellaSequentialBlock(InstellaBlock):
    """
...
class InstellaLlamaBlock(InstellaBlock):
    """
    This is a transformer block where the output is computed as ``MLP(LN(x + Attention(LN(x))))``
    (plus another skip connection). This block is similar to `InstellaSequentialBlock`
...
class InstellaOutput(NamedTuple):
...
class InstellaGenerateOutput(NamedTuple):
...
class InstellaBlockGroup(nn.ModuleList):
...
class Instella(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if self.config.alibi and self.config.flash_attention:
            raise InstellaConfigurationError("ALiBi is currently not supported with FlashAttention")

        if self.config.alibi and self.config.rope:
            raise InstellaConfigurationError("ALiBi and RoPE are mutually exclusive")

        if self.config.embedding_size is not None and self.config.embedding_size != self.config.vocab_size:
            if self.config.embedding_size < self.config.vocab_size:
                raise InstellaConfigurationError("embedding size should be at least as big as vocab size")
        ...
        if not (
            ...
            raise InstellaConfigurationError("n layers must be divisible by block group size")
        ...
        blocks = [InstellaBlock.build(i, config, self.__cache) for i in range(config.n_layers)]
        if self.config.block_group_size > 1:
            block_groups = [
                InstellaBlockGroup(config, i, blocks[i : i + config.block_group_size])
    ...
    def get_fsdp_wrap_policy(self, wrap_strategy: Optional[FSDPWrapStrategy] = None):
        ...
        if wrap_strategy == FSDPWrapStrategy.by_block:
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                ...
                wrap = isinstance(module, InstellaBlock)
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, (InstellaBlock,)) or module in size_based_module_to_wrap
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, InstellaBlockGroup)
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, (InstellaBlockGroup,)) or module in size_based_module_to_wrap
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, InstellaBlock) and module.layer_id % c == 0
                if recurse:
                    return True
                else:
                    return wrap
    ...
    def from_checkpoint(
        ...
        """
        Load an Instella model from a checkpoint.
        """
