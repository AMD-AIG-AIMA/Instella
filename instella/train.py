...
from .data import IterableDataset, SortByLengthDataset
from .eval import Evaluator
from .exceptions import InstellaConfigurationError
from .model import Instella
...
class SpeedMonitor:
    ...
    def batch_start(
        ...
        # num_fwd_flops and num_bck_flops from the Instella model computes flops per token
...
class Trainer:
    ...
    def dataset(self) -> IterableDataset:
        assert isinstance(self.train_loader.dataset, IterableDataset) or isinstance(self.train_loader.dataset, SortByLengthDataset)
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any], load_path: PathOrStr) -> None:
        # Checkpoint paths.
        self.checkpoints = [
            path
        ...
        if isinstance(self.dataset, SortByLengthDataset):
            try:
                dataset_state_dict = json.load(open(os.path.join(load_path, "streaming_dataset_state.json")))
            except:
                dataset_state_dict = None
                log.error(f'Error loading {os.path.join(load_path, "streaming_dataset_state.json")}. Dataloader is not resumed')
                assert self.global_train_examples_seen_this_epoch == 0, "Dataloader needs to be resumed"

            if dataset_state_dict is not None:
                # Load the dataset state
                log.info(f"Resume streaming dataset state from {load_path}: {dataset_state_dict}")
                self.dataset.load_state_dict(dataset_state_dict)
        else:
    ...
    def _save_checkpoint(
        ...
        if isinstance(self.dataset, SortByLengthDataset):
            if get_fs_local_rank() == 0:
                if self.global_train_examples_seen_this_epoch == 0:
                    log.info(f"The streaming dataset has not been iterated. No need to save")
                else:
                    # Save streaming dataset state
                    dataset_state_dict = self.dataset.state_dict(self.global_train_examples_seen_this_epoch, True)
                    log.info(f"Saving streaming dataset state: {dataset_state_dict}")
                    json.dump(dataset_state_dict, open(os.path.join(checkpoint_dir, "streaming_dataset_state.json"), "w"))
    ...
    def restore_sharded_checkpoint(
        ...
        *,
        ...
        if load_trainer_state:
            self.load_trainer_state_dict(trainer_state, load_path)
        barrier()
    ...
    def model_forward(
        ...
        logits = self.dist_model(
            ...
            seq_parallel_group=batch.get("seq_parallel_group"),
            seqlen_offset=batch.get("seqlen_offset", 0),
        ).logits
    ...
    def train_micro_batch(
        ...
        if self.cfg.seq_parallel_size > 1:
            micro_batch = self.get_sequence_parallel_inputs(micro_batch)
    ...
    def get_sequence_parallel_inputs(self, inputs):
        assert inputs["input_ids"].shape[0] == 1 and inputs["doc_lens"].shape[0] == 1, "Multi-sequence sequence parallel is not supported"
        inputs["input_ids"] = inputs["input_ids"].squeeze(0)
        inputs["doc_lens"] = inputs["doc_lens"].squeeze(0)

        seq_parallel_world_size = dist.get_world_size(self.seq_parallel_group)

        assert dist.is_initialized() and seq_parallel_world_size > 1

        seq_parallel_rank = dist.get_rank(self.seq_parallel_group)

        input_ids = inputs["input_ids"]

        doc_lens = inputs["doc_lens"]
        max_doc_lens = inputs["max_doc_lens"]

        assert input_ids.size(-1) % seq_parallel_world_size == 0, "input_ids size should be evenly divided by seq_parallel_world_size"

        if "label_mask" in inputs:
            label_mask = inputs["label_mask"].squeeze(0)
            label_mask_chunks = torch.tensor_split(label_mask, seq_parallel_world_size, dim=-1)
        else:
            label_mask_chunks = None
        ...
        if label_mask_chunks is not None:
            inputs["label_mask"] = label_mask_chunks[seq_parallel_rank]

        max_seq_length = doc_lens.max()
        max_tokens_per_device = doc_lens.sum() // seq_parallel_world_size
        ...
        if max_seq_length <= max_tokens_per_device and not self.cfg.force_sequence_parallel:
            ...
            doc_lens = seq_indices.diff(prepend=start_index_tensor, append=end_index_tensor)
            doc_lens = doc_lens[doc_lens > 0]
            inputs["doc_lens"] = doc_lens
            inputs["max_doc_lens"] = [int(doc_lens.max())]
            inputs["seq_parallel_group"] = None
        
        inputs["input_ids"] = inputs["input_ids"].unsqueeze(0)
        inputs["doc_lens"] = inputs["doc_lens"].unsqueeze(0)
        if label_mask_chunks is not None:
    ...
    def train_batch(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        batch_size_in_tokens = batch["input_ids"].numel() / self.cfg.seq_parallel_size
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        if torch.isnan(ce_batch_loss):
            ...
            log.warning("nan loss encountered")
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    self.global_step += 1
                    self.global_train_examples_seen_this_epoch += global_batch_size // self.cfg.seq_parallel_size
                    self.global_train_tokens_seen += global_batch_size * (seq_len // self.cfg.seq_parallel_size)
                    speed_monitor.batch_start(
                        global_total_tokens=self.global_train_tokens_seen,
                        device_batch_num_tokens=batch_size * seq_len / self.cfg.seq_parallel_size,  # num tokens in batch for this device
