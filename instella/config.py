...
class BaseConfig:
    ...
    def new(cls: Type[C], **kwargs) -> C:
        ...
        try:
            ...
            raise InstellaConfigurationError(str(e))
...
class DataConfig(BaseConfig):
    ...
    dataset_type: str = "MemMapDataset"
...
class DistributedStrategy(StrEnum):
    ddp = "ddp"
    """
    Wrap Instella in torch.nn.parallel.DistributedDataParallel to train across ranks.
    """

    fsdp = "fsdp"
    """
    Wrap Instella in torch.distributed.fsdp.FullyShardedDataParallel to train across ranks.
    """
...
class FSDPWrapStrategy(StrEnum):
    by_block = "by_block"
    """
    Wrap each Instella block with its own FSDP instance.
    """
...
class TrainConfig(BaseConfig):
    """
    Instella training configuration.
    """
    ...
    """
    Distributed strategy for Instella model (eg. single GPU, DDP, FSDP).
    """
    ...
    """
    Deprecated, HF datasets are now stored in `instella_data.hf_datasets`.
    ...
    force_sequence_parallel: bool = False
